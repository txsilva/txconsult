...
if (( $my_query->have_posts() )) {
	if($indice == 0){
		...
		echo '<script> chars['.$i.'] = "<a href=/maraberto/?page_id=2387&letra='.$first_char[$i].' id=custom>'.$first_char[$i].'</a>";</script>';
		
    while ($my_query->have_posts()) : $my_query->the_post(); ?>
	<?php //the_permalink() ?>
        <p><a href="/maraberto/glossario-por-letra/?id=<?php the_id(); ?>&letra=<?php echo $first_char[$i]; ?>" rel="bookmark" title="Verbete: <?php the_title_attribute(); ?>"><?php the_title(); ?></a></p>
        <?php
    endwhile;
	echo '</div>';
